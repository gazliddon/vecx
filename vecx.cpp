...
unsigned char read8(unsigned address) {
	...
	if (address & 0x800) {
		...
		case 0x0:
			...
			data = (unsigned char)((via_orb & 0x5f) | via_t1pb7 |
					       alg_compare);
		    } else {
...
static einline void alg_addline(long x0, long y0, long x1, long y1,
	...
	if (index >= 0 && index < vector_erse_cnt &&
	    x0 == vectors_erse[index].x0 && y0 == vectors_erse[index].y0 &&
...
static einline void alg_sstep(void) {
	...
	if (sig_blank == 0) {
	    ...
	    alg_addline(alg_vector_x0, alg_vector_y0, alg_vector_x1,
			alg_vector_y1, alg_vector_color);
